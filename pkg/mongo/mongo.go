...
type Database interface {
	...
	ObjectIDFromHex(string) (primitive.ObjectID, error)
}
...
type Collection interface {
	...
	SoftDeleteOne(context.Context, interface{}) (*mongo.UpdateResult, error)
...
// Add this interface to pkg/mongo/mongo.go
...
func (md *mongoDatabase) ObjectIDFromHex(id string) (primitive.ObjectID, error) {
...
func (mc *mongoCollection) SoftDeleteOne(ctx context.Context, filter interface{}) (*mongo.UpdateResult, error) {
	result, err := mc.coll.UpdateOne(ctx, filter, bson.M{"$set": bson.M{"deleted_at": time.Now()}})
	return result, err
}
